...
type StageProperties = 'entryId' | 'description' | 'texturePath' | 'name' | 'author';
type StageData = { [key in StageProperties as string]?: string };
type Dictionary<T> = { [key: string]: T };

async function buildEmbedStageMessage(data: StageData) {
...
async function refreshStages(botClient: Discord.Client, guildId: string) {
	...
	const elementVars: StageData = {
		"entryId": "=elements.",
		"description": "* desc=\"",
		"texturePath": "* image=\"",
		"name": "STAGE_REGISTER.register(\"",
		"author": "* author="
	} as StageData;
	if (typeof elementsResponse.data === "string") {
		...
		for (let entry of searchResult) {
			...
			let elementData: StageData = {} as StageData;
			...
			let embed = await buildEmbedStageMessage(elementData);
